...
for i = 1 : size(Returns, 1)
    ...
    for j = 1 : No_Assets
        ...
        Returns_demeaned = Returns_demeaned(~isnan(Returns_demeaned));
        %Returns_demeaned = Returns_demeaned(~isnan(Returns_demeaned));  % Remove NaN values
        if isempty(Returns_demeaned)
            Forecast_Variance(j,1) = 999999999;   %make variance very high if NaN so we don't invest in it
    ...
    if i==1
        TC = sum(abs(weights_MV))*0.0005;
    else 
        TC = sum(abs(weights_MV-weights_MV_TC))*0.0005;
    end
    
    weights_MV_TC = weights_MV;

    Returns_adj = Returns(i,:);
    Returns_adj(isnan(Returns_adj)) = 0;
    Ret_GARCH_MV = Returns_adj * weights_MV - TC;
    Wealth(i+1,1) = Wealth(i,1) * exp(Ret_GARCH_MV);
