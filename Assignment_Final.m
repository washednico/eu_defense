...
Returns_for_corr(any(isnan(Returns_for_corr), 2), :) = [];    %calculate correlation matrix starting from when we have data for all stocks
...
[w_MV_f, sr_MV] = fmincon(objective, weights_MV, A, b, Aeq, beq, lb, ub, [], options);
...
for i = 1 : size(Returns, 1)
    ...
    for j = 1 : No_Assets
        ...
        if isempty(Returns_demeaned)
            Forecast_Variance(j,1) = 999;   %make variance very high if NaN so we don't invest in it
...
% Average absolute annual excess returns last 12 months 
Avg_annual_ew_last12 = exp(mean(Returns_ew_excess(end-12:end))*12)-1;
Avg_annual_GARCH_MV_last12 = exp(mean(Returns_GARCH_MV_excess(end-12:end))*12)-1;

% Average absolute annual excess returns last 6 months 
Avg_annual_ew_last6 = exp(mean(Returns_ew_excess(end-6:end))*12)-1;
Avg_annual_GARCH_MV_last6 = exp(mean(Returns_GARCH_MV_excess(end-6:end))*12)-1;
...
% Estimate betas
b_ew = regress(Returns_ew_excess, [ones(length(Returns_GARCH_MV_excess), 1), Market]);
b_GARCH_MV = regress(Returns_GARCH_MV_excess, [ones(length(Returns_GARCH_MV_excess), 1), Market]);

Treynor_ew = mean(Returns_ew_excess)*12/b_ew(2);
Treynor_GARCH_MV = mean(Returns_GARCH_MV_excess)*12/b_GARCH_MV(2);
...
reg1 = regress(Returns_ew_excess, [ones(length(index), 1), index]);
reg2 = regress(Returns_GARCH_MV_excess, [ones(length(index), 1), index]);
